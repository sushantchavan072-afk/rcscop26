...
const Events = () => {
  ...
  const EventCard = ({ event, index }: { event: typeof EVENTS_DATA[0]; index: number }) => (
    ...
    >
      <Card 
        className="overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer group hover:scale-105"
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {event.avenue && event.avenue !== "NA" && (
              <div className="flex items-center gap-1 text-sm text-muted-foreground">
                ...
                <span className="truncate max-w-[100px]">{event.avenue}</span>
              </div>
            )}
          </div>
          ...
          <>
            <div className="relative h-80 -mx-6 -mt-6 mb-6 overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <DialogTitle className="text-3xl font-bold text-foreground">{selectedEvent.title}</DialogTitle>
              </div>
            </div>
